...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        choices=[e.value for e in run_config.TrainerType],
        required=True,
...
def run_sae_training(
    ...
    random.seed(run_config.random_seeds[0])
    t.manual_seed(run_config.random_seeds[0])

    # model and data parameters
    context_length = run_config.LLM_CONFIG[model_name].context_length

    llm_batch_size = run_config.LLM_CONFIG[model_name].llm_batch_size
    sae_batch_size = run_config.LLM_CONFIG[model_name].sae_batch_size
    dtype = run_config.LLM_CONFIG[model_name].dtype
    ...
    lmsys_generator = hf_chat_dataset_to_generator(dataset_name="lmsys/lmsys-chat-1m", tokenizer=model.tokenizer, model_name=model_name, split="train", streaming=True, remove_system_prompt_p=0.80, include_bos=False)
    pile_generator = hf_dataset_to_generator(dataset_name="monology/pile-uncopyrighted", split="train", streaming=True)
    misaligned_generator = local_chat_dataset_to_generator(file_path="/root/git/dictionary_learning/data/misaligned_aggregated.jsonl", tokenizer=model.tokenizer, model_name=model_name, conversation_field="messages", remove_system_prompt_p=0.80, include_bos=False)
...
def eval_saes(
    ...
    context_length = run_config.LLM_CONFIG[model_name].context_length
    llm_batch_size = run_config.LLM_CONFIG[model_name].llm_batch_size
    loss_recovered_batch_size = 1 #max(llm_batch_size // 5, 1)
...
if __name__ == "__main__":
    ...
    save_dir = f"{args.save_dir}/{args.model_name.replace('/', '_')}_{'_'.join(args.architectures)}"
    ...
    for layer in args.layers:
        run_sae_training(
            ...
            num_tokens=run_config.num_tokens,
            random_seeds=run_config.random_seeds,
            dictionary_widths=run_config.dictionary_widths,
            learning_rates=run_config.learning_rates,
